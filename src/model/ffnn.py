...
class FFNN:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ), f"Weight initialization mode '{weight}' not recognized. Choose from {valid_weights}."
        assert (
            loss in valid_losses
        ), f"Loss function '{loss}' not recognized. Choose from {valid_losses}."
        ...
        assert (
            active_funcs[-1] == "linier"
        ), "Output Layer must use Linier Activation Function."
    ...
    def __getLossFunction(self, loss: str) -> callable:
        ...
        loss_functions = {"mse": LossFunction.mse}
